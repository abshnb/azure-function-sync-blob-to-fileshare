...
def test_function(myblob: func.InputStream):
    ...
    if BLOB_SERVICE_URI:
        try:
            ...
            # Example: list blobs in the container (optional)
            container_client = blob_service_client.get_container_client(BLOB_STORAGE_CONTAINER_NAME)
            blob_list = [b.name for b in container_client.list_blobs(name_starts_with=None)]
            logging.info(f" -> Container blob count sample: {len(blob_list)}")
